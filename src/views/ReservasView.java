...
public class ReservasView extends JFrame {
    ...
    private void guardaReserva() {
        ...
        String formaP = txtFormaPago.getSelectedItem().toString();
        reservacontroller = new ReservaController();
        int id = reservacontroller.guarda(new Reserva(java.sql.Date.valueOf(fechaE), java.sql.Date.valueOf(fechaS),
                valor, formaP));
    ...
    private void calculaValor(JDateChooser txtFechaEntrada1, JDateChooser txtFechaSalida1) {
        if ((txtFechaSalida.getCalendar() != null) && (txtFechaEntrada.getCalendar() != null)) {
            int valor = calculaFecha(txtFechaEntrada1, txtFechaSalida1);
            txtValor.setText(valor + "");
        }
    }

    private int calculaFecha(JDateChooser txtFechaEntrada1, JDateChooser txtFechaSalida1) {
        SimpleDateFormat form = new SimpleDateFormat("yyyy-MM-dd");
        String di = form.format(txtFechaEntrada1.getCalendar().getTime());
        String df = form.format(txtFechaSalida1.getCalendar().getTime());
        LocalDate fi = LocalDate.parse(di);
        LocalDate ff = LocalDate.parse(df);
        Duration dd = Duration.between(fi.atStartOfDay(), ff.atStartOfDay());
        int d = (int) ((dd.toDays() + 1) * 20);
        return d;
    }
}
